...
class ChatUser(generic.TemplateView):
	template_name = "chat/chat_user.html"
...
# 	template_name = "chat/chat-inbox.html"
...
def room(request):
	if 'id' in request.GET:
		id_of_user=request.GET['id']
	# template_name = "chat/chat-inbox.html"
		userprofile_instance = get_object_or_404(UserProfile,id=id_of_user)
		...
		Context={
				'get_chat':get_chat,
				'userprofile_instance':userprofile_instance,
		}

	return render(request, "chat/chat-inbox.html",Context)

@login_required
def messagesubmit(request):
	
	if request.method == 'POST': 
		print(request.POST['message'],request.POST['sender'],request.POST['reciever'])
