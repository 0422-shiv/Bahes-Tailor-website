	...
	def get(self, request, *args, **kwargs):
		...
		if 'service' in request.GET and 'sort-by' in request.GET :
			...
			if q_sort_by == 'Name':
				...
				page = request.GET.get('page', 1)
				paginator = Paginator(get_tailor_sp, 10)
		...
		return render(request, self.template_name,{'get_tailor_sp':get_tailor_sp,'q_sort_by':q_sort_by,'get_tailor_spec':get_tailor_spec,'get_tailor':get_tailor,'get_services':get_services,'get_tailor_specilization':get_tailor_specilization,'get_system_settings':get_system_settings})
...
def get_supplier(data):
	...
	if SupplierServices.objects.filter(tailor_speci_id=data).exists():
		...
		get_supplier_data=SupplierServices.objects.filter(tailor_speci_id=data)
		get_supplier_count=SupplierServices.objects.filter(tailor_speci_id=data).count()
